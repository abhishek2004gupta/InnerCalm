        ...
        <div className="summary-sidebar">
          ...
          <div className="sidebar-section">
            ...
            <div className="youtube-videos">
              {youtubeVideos.length === 0 && <p>No related videos found.</p>}
              {youtubeVideos.map(video => (
                <a key={video.id} href={video.link} target="_blank" rel="noopener noreferrer" style={{ display: 'block', marginBottom: '1rem' }}>
                  <img src={video.thumbnail} alt={video.title} style={{ width: '100%', borderRadius: '8px' }} />
                  <div style={{ fontWeight: 'bold', marginTop: '0.5rem' }}>{video.title}</div>
                </a>
              ))}
            </div>
          </div>
